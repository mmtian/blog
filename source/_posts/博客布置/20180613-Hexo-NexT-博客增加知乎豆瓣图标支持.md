...
## 3. 配置文件参考
因为阿里巴巴矢量库里有多个知乎、豆瓣的图标，大小不一，即使设置了字体大小页可能无法和原始图像大小一致，这让强迫症的我不能忍，经过仔细挑选我终于找到了合适大小的两个图标。

【改进】在 `/themes/next/source/css/_custom/custom.styl` 文件中可以直接加入以下内容：
...
【改进】此外，主题配置文件中，社交账号图标设置好以后，类似是以下这样的格式：
...
~~如果大家需要在侧边栏显示知乎、豆瓣，在 `/themes/next/source/css/_custom/custom.styl` 文件中可以直接加入以下内容：~~
...
~~主题配置文件中，社交账号图标设置好以后，类似是以下这样的格式：~~
